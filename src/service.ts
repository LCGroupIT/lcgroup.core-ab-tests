...
export class AbTestsService {
    ...
    private _rootTestsStarted$: ReplaySubject<TestStartedEvent> = new ReplaySubject();
    ...
    constructor(
        ...
        for (const config of configs) {
            ...
            this._rootTestsStarted$.next(new TestStartedEvent(scope, this.getVersion(scope)));
        }
    }
    ...
    setTest(scope: string, configOrVersions: AbTestOptions | string[]) {
        if (!!this._tests[scope]) {
            return;
        }
    ...
    setTestAndEmitStarted(scope: string, configOrVersions: AbTestOptions | string[]) {
        this.setTest(scope, configOrVersions);
        this.emitTestStarted(scope, this.getVersion(scope));
    }

    private getConfig(configOrVersions: AbTestOptions | string[]): AbTestOptions {
        return Array.isArray(configOrVersions) ? {versions: configOrVersions} : configOrVersions;
    }

    private setupScope(isCrawler: boolean, scope: string, config: AbTestOptions) {
        if (isCrawler) {
    ...
    private setupTestForCrawler(scope: string, config: AbTestOptions) {
    ...
    private setupTestForRealUser(scope: string, config: AbTestOptions) {
